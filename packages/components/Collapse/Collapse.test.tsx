...
describe('Collapse.vue', () => {
  beforeAll(() => {
    ...
    headers = wrapper.findAll(collapseItemBem.e('header', true))
    contents = wrapper.findAll(collapseItemBem.e('wapper', true))
  ...
  test('测试基础结构以及对应文本', () => {
    ...
    // content
    expect(firstHeader.classes()).toContain(collapseItemBem.is('active'))
    expect(firstContent.isVisible()).toBeTruthy()
    expect(secondHeader.classes()).not.toContain(collapseItemBem.is('active'))
  ...
  test('点击标题展开/关闭内容', async () => {
    ...
    expect(secondHeader.classes()).toContain(collapseItemBem.is('active'))
    expect(secondHeader.isVisible()).toBeTruthy()
    expect(firstHeader.classes()).not.toContain(collapseItemBem.is('active'))
    expect(firstContent.isVisible()).toBeFalsy()
  })
  ...
  test('disabled 内容', async () => {
    // disabled
    expect(disabledHeader.classes()).toContain(collapseItemBem.is('disabled'))
    onChange.mockClear()
  ...
  test('手风琴模式 错误处理', () => {
    ...
    expect(warn.mock.calls).toMatchInlineSnapshot(
      `
        [
          [
            ${createDebugWarnMessage(
              COMP_NAME,
              'accordion mode can only support one item active'
            )},
          ],
        ]
      `
