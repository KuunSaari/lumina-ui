...
describe('Alert.vue', () => {
  ...
  it('should support centering text', () => {
    ...
    //class
    const rootNode = wrapper.find(bem.getName(true))
    expect(rootNode.classes()).toContain('is-text-center')
  })
  ...
  it('should not render close icon when closable is false', () => {
    ...
    expect(wrapper.find(bem.e('close', true)).exists()).toBe(false)
  })
  ...
  it('should toggle visibility when open and close methods are called', async () => {
    ...
    expect(wrapper.find(bem.getName(true)).attributes().style).toBe('display: none;')
    await wrapper.vm.open()
    expect(wrapper.find(bem.getName(true)).attributes().style).toBe('')
  })
})
